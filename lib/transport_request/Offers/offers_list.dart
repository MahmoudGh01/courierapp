...
class _OffersListPageState extends State<OffersListPage> {
  ...
  void initState() {
    ...
    _fetchFuture = Provider.of<OfferProvider>(context, listen: false).fetchOffersT(widget.requestId);
  }
                  ...
                  onRetry: () {
                    setState(() {
                      _fetchFuture = Provider.of<OfferProvider>(context, listen: false)
                          .fetchOffersT(widget.requestId); // retry
                    });
                  },
                );
              }
  ...
  Widget _buildOffersList(BuildContext context, List<OfferModel> offers) {
    ...
    return RefreshIndicator(
      onRefresh: () async {
        ...
        await p.fetchOffersT(widget.requestId);
        ...
        itemBuilder: (context, index) {
          ...
          return GestureDetector(
            onTap: () async {
              ...
              if (updated == true) {
                await p.fetchOffersT(widget.requestId); // refresh après accept/refuse
                setState(() {}); // rebuild
              }
            },
